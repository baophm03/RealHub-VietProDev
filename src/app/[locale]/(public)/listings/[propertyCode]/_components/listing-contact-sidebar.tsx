...
export function ListingContactSidebar(props: ListingContactSidebarProps) {
  return (
    <Suspense fallback={<ListingContactSidebarInner {...props} />}>
      <ListingContactSidebarInner {...props} />
    </Suspense>
  );
}

function ListingContactSidebarInner({
  propertyId,
  ...
  const handleBookConsultation = async () => {
    ...
    try {
      const result: any = await consultationMutation.mutateAsync({
        data: { propertyId, recipientUserId } as any,
      });
    ...
    <div className="w-full lg:w-1/3 lg:sticky lg:top-24">
      <div className="bg-surface rounded-xl border border-border shadow-sm p-6 space-y-6">
        ...
        <div className="space-y-3">
          ...
          {contacts.map((item) => (
            <div key={item.name ?? item.phone} className="flex flex-col gap-3">
              {item.phone && (
