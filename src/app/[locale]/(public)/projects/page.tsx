...
import { mockProjects, mockProjectCategories, mockProjectStatuses } from "@/lib/mock/projects";
import { MapPin, Building, House, ArrowRight, Funnel, CheckCircle } from "@phosphor-icons/react";
...
export default function ProjectsPage() {
  ...
  const filtered = mockProjects.filter((p) => {
    if (selectedCategory !== "Tất cả" && !p.propertyTypes.includes(selectedCategory)) return false;
    return true;
  });
              ...
              <div className="flex flex-1 flex-col gap-3 p-5">
                ...
                <div className="flex items-center justify-between border-t border-border pt-3">
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    <span className="text-sm font-medium text-foreground">{project.scale}</span>
                  </div>
                </div>

                <div className="flex items-center gap-3 text-xs text-foreground-muted">
                  <span className="flex items-center gap-1">
                    <Building size={12} /> {project.investor}
                  </span>
                  <span className="flex items-center gap-1">
                    <House size={12} /> {project.propertyTypes[0]}
                  </span>
                </div>

                <div className="mt-auto flex items-center justify-between border-t border-border pt-3">
                  <span className="text-xs text-foreground-muted">Bàn giao: {project.handover}</span>
                  <ArrowRight
                    size={16}
