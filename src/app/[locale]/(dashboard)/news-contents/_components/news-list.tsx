      ...
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-1">
          <button
            onClick={(e) => {
              e.stopPropagation();
              router.push(`/news-contents/news/${row.original.id}/edit`);
            }}
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {mounted && hasPermission("news:write") && (
          <Button onClick={() => router.push("/news-contents/news/new")}>
            <Plus size={16} />
            Thêm bài viết
          </Button>
        )}
      ...
      {isLoading ? (
        ...
        <DataTable
          columns={columns}
          data={filtered}
          onRowClick={(row) => router.push(`/news-contents/news/${row.id}/edit`)}
